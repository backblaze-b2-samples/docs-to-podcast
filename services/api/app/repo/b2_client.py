...
def get_s3_client():
    return boto3.client(
        ...
        # Region comes from config (B2_REGION) — never hardcoded in source.
        region_name=settings.b2_region or None,
        aws_access_key_id=settings.b2_application_key_id,
        aws_secret_access_key=settings.b2_application_key,
        config=Config(
            signature_version="s3v4",
            user_agent_extra="b2ai-docs-to-podcast",
        ),
    )
...
def get_object_bytes(key: str) -> bytes | None:
    """Read an object's raw bytes back from B2.

    Returns None if the key does not exist; raises RuntimeError on other
    S3 failures. Used by the generation pipeline to fetch source documents
    that were uploaded earlier (generation is decoupled from upload), and
    by the show/episode manifest helpers below.
    """
    client = get_s3_client()
    try:
...
def get_json(key: str) -> dict | None:
    """Read and parse a JSON object from B2. Returns None if absent."""
...
def put_json(key: str, data: dict) -> None:
    """Serialize and write a JSON object to B2 (e.g. a show/episode manifest)."""
...
def list_prefixes(prefix: str) -> list[str]:
    """List the immediate child 'folders' (common prefixes) under a prefix.

    Uses a delimiter so we get one entry per show rather than every object.
    ...
    try:
        ...
        raise RuntimeError(f"B2 prefix list failed: {e}") from e
    return prefixes
...
def get_inline_presigned_url(key: str, expires_in: int = 3600) -> str:
    """Presigned URL for inline playback (no attachment disposition).

    Used by the Shows library HTML5 audio player to stream episode MP3s.
    """
    client = get_s3_client()
    params: dict = {"Bucket": settings.b2_bucket_name, "Key": key}
    try:
