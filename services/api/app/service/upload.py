...
# Source documents fed to the LLM are restricted to readable text formats.
# Markdown arrives as text/plain or text/markdown depending on the browser.
DOC_TYPES = {
    "application/pdf",
    "text/plain",
    "text/markdown",
...
def process_upload(
    ...
    *,
    ...
    """Validate and store an uploaded file. Raises UploadError on failure.

    `key_prefix` controls where the object lands (default `uploads/` for the
    generic Upload page; show-scoped ingestion passes `shows/{id}/sources/`).
    `allowed_types` narrows the accepted MIME set (e.g. DOC_TYPES for sources).
    Both keep show ingestion DRY against the generic uploader rather than
    duplicating the validation pipeline.
    """
    type_allowlist = allowed_types if allowed_types is not None else ALLOWED_TYPES
    ...
    if content_type not in type_allowlist:
        raise UploadError(
