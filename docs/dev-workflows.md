...
- **Generation**: the pipeline is tested with **mocked** LLM/TTS adapters
  (`tests/test_generation.py`). Tests monkeypatch `generate_script` /
  `synthesize_line` and the B2 manifest helpers, so there are **no real API
  calls, no network, and no keys required in CI**.
...
- E2E: `pnpm test:e2e` (run `pnpm --filter @docs-to-podcast/web exec playwright install chromium` once first)
...
Both script generation and audio synthesis use **OpenAI** through a single
SDK and a single `OPENAI_API_KEY`. The adapters live in `services/api/app/repo/`
(`llm_client.py`, `tts_client.py`) — never call the SDK from `service/` or
`runtime/`. Model and voice defaults are env-configurable:

- `LLM_MODEL` (default `gpt-4o-mini`) — script generation
- `TTS_MODEL` (default `gpt-4o-mini-tts`) — audio synthesis
- `TTS_VOICE_HOST_A` / `TTS_VOICE_HOST_B`, `HOST_A_NAME` / `HOST_B_NAME`

To swap providers (e.g. ElevenLabs for TTS), change only the relevant `repo/`
adapter — the rest of the pipeline only knows it gets a script back / MP3 bytes
back. When working on generation, **test with the mocked adapters** rather than
