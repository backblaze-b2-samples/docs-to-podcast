...
## Purpose
List, preview, download, and delete files stored in Backblaze B2. This is the **whole-bucket** explorer over *every* object — generic uploads plus the show sources, transcripts, and episode audio written by the podcast pipeline. The Shows library (`/shows`) is the complementary explorer scoped to the `shows/` prefix.
