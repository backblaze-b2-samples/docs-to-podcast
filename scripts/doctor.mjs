...
// OpenAI key powers both LLM script generation and TTS — required to generate
// episodes, but the app + tests boot without it, so warn rather than fail.
const OPTIONAL_VARS = ["OPENAI_API_KEY"];
const OPTIONAL_PLACEHOLDERS = new Set(["your_openai_api_key"]);
...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-docs-to-podcast)",
    );
  }
  // OpenAI key is needed to generate episodes (LLM + TTS). Missing/placeholder
  // is a warning: the rest of the app (uploads, Files explorer, dashboard) and
  // the test suite all work without it.
  const openaiMissing = OPTIONAL_VARS.filter(
    (k) => !env[k] || OPTIONAL_PLACEHOLDERS.has(env[k]),
  );
  if (openaiMissing.length > 0) {
    warn(
      `.env is missing OpenAI config: ${openaiMissing.join(", ")}`,
      "Episode generation (script + audio) needs OPENAI_API_KEY. Get one at https://platform.openai.com/api-keys, then add it to .env. Uploads, the Files explorer, and the dashboard work without it.",
    );
  }
}
