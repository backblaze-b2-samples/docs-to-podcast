...
import { useGenerationActivity } from "@/lib/queries";
...
export function GenerationChart() {
  const { data: activity, error, refetch } = useGenerationActivity(7);
  ...
  return (
    <Card>
      <CardHeader className="border-b border-border py-4 px-5">
        <CardTitle className="card-title">Generation Activity</CardTitle>
        <CardDescription className="text-xs">Episodes per day, last 7 days</CardDescription>
      ...
      <CardContent className="p-5">
        {error ? (
          ...
          <EmptyState
            ...
            description="Generate an episode to see activity trends here."
          />
        ) : (
